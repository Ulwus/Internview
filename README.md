...
## Setup Guide
Proje şu anda **Hafta 3 (API Gateway)**, **Hafta 4 (Auth Service)** ve **Hafta 5 (User Service)** aşamalarını tamamlamış durumdadır. Gateway tarafında route tanımları, basic authentication filter, rate limiting ve loglama; auth tarafında Spring Security, HS256 JWT (üretim/doğrulama), Flyway ile PostgreSQL şeması, `register` / `login` / `refresh` uçları ve rol tabanlı erişim (RBAC); user tarafında ise `User`, `ExpertProfile`, `Skill`, `Industry` domain modeli, JPA Specification tabanlı filtreleme + pagination ile `GET /experts`, `GET /experts/{id}`, `GET /experts/me`, `PUT /experts/me`, `GET|PUT /users/profile`, `GET /industries`, `GET /skills` uçları ve servis/controller slice testleri bulunmaktadır. Geliştirme ortamı Docker Compose ile PostgreSQL, Redis, Kafka ve Consul servislerini tek komutla ayağa kaldıracak şekilde yapılandırılmıştır. Klasör iskeleti aşağıdaki gibidir:
